...
# --- Bootstrap --------------------------------------------------------------------
    # _framework_locator
        ...
        #
        #       SGND_FRAMEWORK_ROOT     [default: /]
        #       SGND_APPLICATION_ROOT   [default: SGND_FRAMEWORK_ROOT]
        #
        ...
        #
        #       SGND_FRAMEWORK_ROOT defaults to /
        #       SGND_APPLICATION_ROOT defaults to SGND_FRAMEWORK_ROOT
        #
        # Returns:
        #   0   success
    ...
    _framework_locator (){
        ...
        if [[ -r "$cfg_user" ]]; then
            ...
            if [[ -t 0 && -t 1 ]]; then
                ...
                read -r -p "SGND_FRAMEWORK_ROOT [/] : " reply
                fw_root="${reply:-/}"

                read -r -p "SGND_APPLICATION_ROOT [/] : " reply
    ...
    # _load_bootstrapper
        ...
        #   roots have been established by _framework_locator.
        #
        # Behavior:
        #   1. Calls _framework_locator to load or create the bootstrap cfg.
        #   2. Derives the bootstrap path from SGND_FRAMEWORK_ROOT.
        #   3. Verifies that td-bootstrap.sh is readable.
        #   4. Sources td-bootstrap.sh into the current shell.
        #
        # Path rule:
        #   If SGND_FRAMEWORK_ROOT is "/":
        #
        #       /usr/local/lib/testadura/common/td-bootstrap.sh
        #
        #   Otherwise:
        #
        #       $SGND_FRAMEWORK_ROOT/usr/local/lib/testadura/common/td-bootstrap.sh
        #
        # Notes:
    ...
    _load_bootstrapper(){
        ...
        if [[ "$SGND_FRAMEWORK_ROOT" == "/" ]]; then
                ...
                bootstrap="${SGND_FRAMEWORK_ROOT%/}/usr/local/lib/testadura/common/td-bootstrap.sh"
            fi
...
# --- Main -------------------------------------------------------------------------
    ...
    main() {
        ...
        while true; do
            ...
            case $rc in
                0) saydebug "0 detected"; SGND_STATE_SAVE=1; sgnd_save_state; break ;;
                1) sayinfo "Redoing selection..."; saydebug "Redo detected"; continue ;;
                2) saywarning "User quit."; SGND_STATE_SAVE=0; saydebug "Quit"; break ;;
