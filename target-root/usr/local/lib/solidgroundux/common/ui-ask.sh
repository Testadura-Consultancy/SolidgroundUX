...
# --- Public API ---------------------------------------------------------------------
        ...
        if (( echo_input )); then
            if (( ok )); then
                printf "  %b%s%b %b✓%b\n" \
                    ...
                    "$SGND_UI_VALID" "$RESET" >/dev/tty
            else
                printf "  %b%s%b %b✗%b\n" \
                    "$inputclr" "$value" "$RESET" \
                    "$SGND_UI_INVALID" "$RESET" >/dev/tty
            fi
        fi
    ...
    ask_choose_immediate() {
        ...
        local labelwidth=""
        local pad=""
        local labelclr="${SGND_UI_LABEL:-${SGND_UI_TEXT:-}}"
        local inputclr="${SGND_UI_INPUT:-${SGND_UI_TEXT:-}}"
        local colorize="both"
