...
- Added the framework-owned `sgnd-smoketest` public command and standalone smoke-test implementation. The Framework owns framework installation validation and interactive framework smoke tests; Management Console registration and module validation remain application concerns.
- Added convenience-template discovery for workspace creation. `create-workspace` can select templates from `usr/local/share/solidgroundux/convenience-templates` and copy the selected templates into the new workspace.
- Added optional per-template starter-script creation during workspace creation.
- Added repository starter material to newly created workspaces: `README.md`, `CHANGELOG.md`, and a project logo beneath `target-root/usr/local/assets`.
- Added per-product removal-baseline selection to `prepare-release`, allowing each selected product and the combined bundle to choose its own historical manifest or explicitly use no removal baseline.

### Changed

- Moved the SolidGround Management Console host (`management-console.sh`) and its public `sgnd-console` command out of the SolidGroundUX Framework and into the independently released Management Console Modules product. The Framework retains the reusable `sgnd-menu` API and runtime services consumed by the console.
- `create-workspace` now treats Product name, Group, Subgroup, and Project root as persistent workspace defaults. Project folder defaults to `<project-root>/<normalized-project-name>`.
- Starter scripts generated by `create-workspace` initialize Version to `1.0`, Build and Checksum to `-`, replace the template product/title identity with the new project identity, and ask for script name, Group, and Subgroup per selected template. Subgroup defaults to empty.
- Workspace completion now displays the generated directory tree and ends with the standard section separator before the final return prompt.
- `prepare-release` now keeps release metadata policy with each selected product, while a bundle inherits Version and Build from the primary product.
- Removal-baseline discovery in `prepare-release` is filtered by product identity rather than presenting manifests belonging to other products.
